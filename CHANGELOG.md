...
- **Report destination in config** (`setting|key|value`): persist `report_path` (saved report file) and/or `desktop_report` (`true`/`false`) in `custom.conf` so unattended runs do not need CLI flags or env vars. Precedence is CLI > environment > config setting > built-in default (`~/.config/tm_exclusions/last_report.txt`). `report_path` is a file path (not a directory), matching `TM_EXCLUSIONS_REPORT`. `#@option` was not used — `#@` lines stay comments reserved for category markers (#34). ([#52](https://github.com/qveys/tm-exclusions/issues/52))
...
- Selective Application Support developer-tool caches ([#53](https://github.com/qveys/tm-exclusions/issues/53)): Cursor `User/workspaceStorage` and Electron caches; Antigravity / Antigravity IDE caches; auto-claude-ui venvs; Discord `Cache` / modules; GitKraken CLI `versions`; JetBrains `*/plugins` and Toolbox `apps`; virtualenv tool cache; SQLTools `node_modules`; Zed `languages` / `extensions` / `node`. Parent Application Support app folders stay in backups (settings, tokens, keymaps). `path` targets may include glob metacharacters (`*`, `?`, `[`), expanded at load time.
- Static paths for `/Applications` and `$HOME/Applications`.
- Default config now ships ~146 rules across 17 categories (up from 39). Path style normalized to `$HOME/...`. New `#@CategoryName` section markers (cosmetic in 1.x; future report grouping tracked in #34). Several entries ship commented (opt-in): `pattern|.cache`, `pattern|site-packages`, `path|$HOME/.docker` (keeps registry credentials). `$HOME/.ollama/models` replaces the parent `$HOME/.ollama` (preserves `id_ed25519` and chat history). Cloud-sync prunes deferred to user opt-in (#44). Parent Application Support roots stay omitted (mix of user data and caches); granular cache subdirs ship enabled (#53). (#37)
